...
namespace glshader::process::impl::macro
{
    ...
    const files::path& current_file, const int current_line, processed_file& processed)
  {
    ...
    if (inputs.size() != info.parameters.size() || (info.parameters.size() >= inputs.size() - 1 && inputs.back() ==
      "..."))
    {
      processed.syntax_error(current_file, current_line, strfmt(strings::serr_non_matching_argc, name.c_str()));
      return "";
    }
    ...
    processed_file& processed)
  {
    ...
    while (true)
    {
      ...
      char const* end_params = skip::space(text_ptr);
	  if(!impl::classify::is_eof(end_params)) {
			end_params--;
	  }
      int params_length = 0;
